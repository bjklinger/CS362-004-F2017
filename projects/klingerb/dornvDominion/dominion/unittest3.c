...
int main(){
	...
	// CONDITION 1: The deck contains cards:
	printf("*** %s FUNCTION CONDITION 1: The Deck contains cards ***\n\n", TESTFUN);
	...
	printf("\n\n*** %s FUNCTION CONDITION 2: The Deck contains no cards ***\n\n", TESTFUN);
...
void unitTestSuite(struct gameState *Game, struct gameState *TestGame){
	...
	// Call the shuffle function to shuffle the player's deck:
	result = shuffle(THIS_PLAYER, TestGame);
	printf("\n\n*** TESTING CONDITION ***\n\n");
	printf("Shuffling the deck:\n\n");
	
	// Return result if it was a successful shuffle:
	if(Game->deckCount[THIS_PLAYER] > 0){
		hasCards = TRUE;
	}
	...
	if(hasCards){
		...
		printf("Return result of calling function: %d; should be: -1\n", result);
		assertTrue(result == -1);
	}

	// Game state: The deck cards should be shuffled and be present in player's deck:
	...
	printf("\nGAME STATE: Player deck should contain the same amount of cards - ");
